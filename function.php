...
function changestatus($f_id, $f_status, $f_info, $f_returntime)
{
    ...
    $result = $db_attendance->query("SELECT timestamp FROM events WHERE studentid='$f_id' ORDER BY timestamp DESC LIMIT 1");
    ...
    $stmt = $db_attendance->prepare("INSERT INTO events (studentid, statusid, info, returntime) VALUES (?, ?, ?, ?)");
...
function favorite($id, $status, $info, $returntime)
{
    ...
    $getfav = $db_attendance->query("SELECT * FROM cookiedata WHERE studentid = '".$id."'");
    $frowcnt =  $getfav->num_rows;

    if ($frowcnt <10) {
        $stmt=$db_attendance->prepare("INSERT INTO cookiedata (studentid, statusid, info, returntime) VALUES (?, ?, ?, ?)");
...
function validDate($v_date)
{
    ...
    global $db_attendance;
    $globalsQuery = $db_attendance->query("SELECT *
                                  FROM globals
                                  ");

    $holidayQuery = $db_attendance->query("SELECT *
                                  FROM holidays
                                  ");
...
{
    global $db_attendance;
    $query = $db_attendance->query("SELECT firstname FROM studentdata WHERE studentid = $id");
...
{
    global $db_attendance;
    $query = $db_attendance->query("SELECT lastname FROM studentdata WHERE studentid = $id");
    $tempvar = $query->fetch_assoc();
...
{
    global $db_attendance;
    $query = $db_attendance->query("SELECT statusname FROM statusdata WHERE statusid = $id");
    $tempvar = $query->fetch_assoc();
...
{
    global $db_attendance;
    $query = $db_attendance->query("SELECT name FROM groups WHERE groupid = $id");
    $tempvar = $query->fetch_assoc();
    $name = $tempvar['name'];
    return($name);
}
...
{
    global $db_attendance;
    $query = $db_attendance->query("SELECT groupid FROM groups WHERE name = $name");
    $tempvar = $query->fetch_assoc();
    $id = $tempvar['groupid'];
    return($id);
}
...
$holiday_result = $db_attendance->query($holidayquery);
...
$result = $db_attendance->query("SELECT info,statusname,studentdata.studentid,studentdata.firstname,timestamp,returntime,events.eventid, yearinschool
		FROM events
		...
		ORDER BY timestamp ASC") or die(mysqli_error($db_attendance));
...
$allotted_result = $db_attendance->query($allottedquery);
$allotted_data_array = $allotted_result->fetch_array();

//globals query
$globalsquery = "SELECT * FROM globals";
$globals_result = $db_attendance->query($globalsquery);
...
$query = $db_attendance->query("SELECT yearinschool FROM studentdata WHERE studentid = $current_student_id");
$tempvar = $query->fetch_assoc();
$studentYis = $tempvar['yearinschool'];

$query = $db_attendance->query("SELECT startdate FROM studentdata WHERE studentid = $current_student_id");
$tempvar = $query->fetch_assoc();
